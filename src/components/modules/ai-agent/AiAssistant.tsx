...
import { Sparkles, Send, Bot, User, Trash2, ArrowUpRight } from "lucide-react";
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <button
          ...
          className="group relative flex items-center gap-2.5 rounded-full border border-brand-500/40 bg-zinc-950/90 px-4 py-3 text-sm font-semibold text-white shadow-[0_0_25px_rgba(79,117,243,0.3)] backdrop-blur-xl transition-all duration-300 hover:scale-105 hover:border-brand-500/80 hover:shadow-[0_0_35px_rgba(79,117,243,0.5)]"
      ...
      <Dialog open={isOpen} onOpenChange={onOpenChange}>
        ...
        >
          {/* Header */}
          <div className="px-6 py-4 border-b border-white/[0.08] bg-zinc-950/80 flex items-center justify-between">
            ...
            <button
              onClick={clearChat}
              className="mr-8 p-1.5 rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-white/10 transition"
              title={t("clear")}
            >
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 text-sm">
            {messages.map((msg) => (
              ...
              >
                ...
                {msg.role === "user" && (
                  <div className="h-7 w-7 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center shrink-0 mt-0.5">
            ...
            {loading && (
              <div className="flex gap-3 justify-start items-center text-xs text-zinc-400 font-mono">
                <div className="h-7 w-7 rounded-lg bg-brand-500/20 border border-brand-500/30 flex items-center justify-center shrink-0">
                  <Bot className="h-3.5 w-3.5 text-brand-400 animate-spin" />
                </div>
          ...
          {/* Quick Prompts */}
          <div className="px-6 py-2 border-t border-white/[0.06] bg-black/20 flex gap-2 overflow-x-auto no-scrollbar">
            {quickPrompts.map((prompt, i) => (
              <button
                key={i}
                onClick={() => handleSend(prompt)}
                disabled={loading}
                className="whitespace-nowrap px-3 py-1 rounded-full border border-white/[0.08] bg-white/[0.03] text-[11px] text-zinc-400 hover:text-white hover:border-brand-500/40 hover:bg-brand-500/10 transition shrink-0"
              >
                {prompt}
              </button>
            ))}
          ...
          {/* Input Form */}
          <div className="p-4 border-t border-white/[0.08] bg-zinc-950/80">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleSend();
            ...
            >
              <Input
                ...
                disabled={loading}
                className="bg-zinc-900/90 border-white/10 text-xs sm:text-sm"
              />
              <Button
                type="submit"
